use crate::types::{SystemState, ScaleData, TimerState, BrewState, AutoTareState, BrewConfig};
...
impl StateManager {
    ...
    pub async fn update_scale_data(&self, scale_data: ScaleData) {
        ...
        self.add_log_message(&mut state, format!("Scale: {:.2}g, {:.2}g/s", weight, flow_rate));
    }
    
    pub async fn update_timer_state(&self, timer_state: TimerState) {
        let mut state = self.state.lock().await;
        if state.timer_state != timer_state {
            info!("Timer state changed: {:?} -> {:?}", state.timer_state, timer_state);
    ...
    pub async fn update_brew_state(&self, brew_state: BrewState) {
        ...
        if state.brew_state != brew_state {
            info!("Brew state changed: {:?} -> {:?}", state.brew_state, brew_state);
    ...
    pub async fn update_auto_tare_state(&self, auto_tare_state: AutoTareState) {
        ...
        if state.auto_tare_state != auto_tare_state {
            debug!("Auto-tare state changed: {:?} -> {:?}", state.auto_tare_state, auto_tare_state);
    ...
    pub async fn set_relay_enabled(&self, enabled: bool) {
        ...
        if state.relay_enabled != enabled {
            info!("Relay state changed: {}", if enabled { "ON" } else { "OFF" });
            state.relay_enabled = enabled;
            self.add_log_message(&mut state, format!("Relay: {}", if enabled { "ON" } else { "OFF" }));
        }
    }
    ...
    pub async fn set_ble_connected(&self, connected: bool) {
        ...
        if state.ble_connected != connected {
            info!("BLE connection changed: {}", if connected { "Connected" } else { "Disconnected" });
            state.ble_connected = connected;
            self.add_log_message(&mut state, format!("BLE: {}", if connected { "Connected" } else { "Disconnected" }));
        }
    }
    ...
    pub async fn set_wifi_connected(&self, connected: bool) {
        ...
        if state.wifi_connected != connected {
            info!("Wi-Fi connection changed: {}", if connected { "Connected" } else { "Disconnected" });
            state.wifi_connected = connected;
            self.add_log_message(&mut state, format!("Wi-Fi: {}", if connected { "Connected" } else { "Disconnected" }));
        }
    }
