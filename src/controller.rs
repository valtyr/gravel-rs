use crate::{
    ble::{BleScaleClient, ScaleDataChannel, BleStatusChannel},
    websocket::{WebSocketServer, WebSocketCommand, WebSocketCommandChannel},
    ...
    types::{SystemState, ScaleData, TimerState, BrewState, BrewConfig},
};
...
impl EspressoController {
    ...
    pub async fn start(&mut self, spawner: Spawner, bt_driver: BtDriver<'static, Ble>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Spawn BLE task
        spawner.spawn(ble_task(ble_client, bt_driver)).map_err(|_| "Failed to spawn BLE task")?;
    ...
    async fn handle_scale_data(&mut self, scale_data: ScaleData) {
        debug!("Received scale data: {:.2}g, {:.2}g/s", scale_data.weight_g, scale_data.flow_rate_g_per_s);
        
        self.safety_controller.update_data_received();
        self.state_manager.update_scale_data(scale_data.clone()).await;
        ...
        if let Some(transition) = self.brew_state_machine.update(&scale_data, self.state_manager.get_timer_state().await) {
        ...
        if self.state_manager.is_auto_tare_enabled().await {
            if let Some(action) = self.auto_tare_controller.update(&scale_data, self.state_manager.get_brew_state().await) {
                self.handle_auto_tare_action(action).await;
            }
            self.state_manager.update_auto_tare_state(self.auto_tare_controller.get_state()).await;
        }
    ...
    async fn handle_brewing_logic(&mut self, scale_data: &ScaleData) {
        ...
        if let Some(timing) = self.overshoot_controller.calculate_stop_timing(
            ...
            if timing.should_stop_now && self.last_stop_prediction.is_none() {
                info!("Predictive stop triggered at {:.2}g (target: {:.2}g, predicted: {:.2}g)",
        ...
        if let Some(prediction_start) = self.prediction_timer_start {
            if Instant::now().duration_since(prediction_start) > Duration::from_secs(10) {
                if let Some(prediction) = &self.last_stop_prediction {
                    self.overshoot_controller.record_actual_result(scale_data.weight_g);
                    info!("Recorded final weight for overshoot learning: {:.2}g", scale_data.weight_g);
                }
    ...
    async fn handle_ble_status_change(&mut self, connected: bool) {
        ...
        if !connected {
            self.state_manager.set_error(Some("BLE disconnected".to_string())).await;
        } else {
    ...
    async fn handle_websocket_command(&mut self, command: WebSocketCommand) {
        ...
        match command {
            ...
            WebSocketCommand::SetAutoTare(enabled) => {
                ...
                self.state_manager.update_config(config).await;
                info!("Auto-tare: {}", if enabled { "enabled" } else { "disabled" });
            }
            ...
            WebSocketCommand::SetPredictiveStop(enabled) => {
                ...
                self.state_manager.update_config(config).await;
                info!("Predictive stop: {}", if enabled { "enabled" } else { "disabled" });
            }
            
            WebSocketCommand::TestRelay => {
                if let Err(e) = self.relay_controller.test_relay().await {
                    warn!("Relay test failed: {:?}", e);
                    self.state_manager.add_log("Relay test failed".to_string()).await;
                } else {
                    self.state_manager.add_log("Relay test completed successfully".to_string()).await;
                }
            }
            ...
            WebSocketCommand::TareScale => {
                if let Err(e) = self.ble_client.send_tare_command().await {
                    ...
                    self.state_manager.add_log("Failed to tare scale".to_string()).await;
                } else {
                    self.state_manager.add_log("Scale tared manually".to_string()).await;
                }
            }
            ...
            WebSocketCommand::StartTimer => {
                if let Err(e) = self.ble_client.send_start_timer_command().await {
                    ...
                    self.state_manager.add_log("Timer started manually".to_string()).await;
                }
            }
            ...
            WebSocketCommand::ResetTimer => {
                if let Err(e) = self.ble_client.send_reset_timer_command().await {
                    ...
                    self.state_manager.add_log("Timer reset manually".to_string()).await;
                }
            ...
            WebSocketCommand::ResetOvershoot => {
                ...
                self.state_manager.add_log("Overshoot controller reset".to_string()).await;
            }
        }
    }
    ...
    async fn periodic_update(&mut self) {
        ...
        self.safety_controller.update_relay_state(current_state.relay_enabled);
    }
    ...
    async fn stop_brewing(&mut self) -> Result<(), RelayError> {
        ...
        self.state_manager.set_relay_enabled(false).await;
        self.state_manager.add_log("Brewing stopped".to_string()).await;
    ...
    async fn emergency_stop(&mut self) {
        ...
        self.safety_controller.handle_emergency_stop(&mut self.relay_controller);
        self.state_manager.set_relay_enabled(false).await;
        self.state_manager.set_error(Some("Emergency stop activated".to_string())).await;
        self.state_manager.add_log("EMERGENCY STOP".to_string()).await;
...
async fn ble_task(ble_client: BleScaleClient, bt_driver: BtDriver<'static, Ble>) {
    info!("BLE task started");
    if let Err(e) = ble_client.start(bt_driver).await {
