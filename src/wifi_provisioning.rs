...
impl WifiProvisioning {
    ...
    pub fn is_provisioned(&self) -> Result<bool, EspError> {
        ...
        if result == ESP_OK {
            ::log::debug!("📋 WiFi provisioning status: {}", if provisioned { "provisioned" } else { "not provisioned" });
            Ok(provisioned)
        } else {
    ...
    pub fn start_provisioning(
        ...
        let pop_cstr = pop.map(|p| CString::new(p))
            .transpose()
            .map_err(|_| EspError::from(ESP_ERR_INVALID_ARG).unwrap())?;
        
        let service_key_cstr = service_key.map(|k| CString::new(k))
            .transpose()
            .map_err(|_| EspError::from(ESP_ERR_INVALID_ARG).unwrap())?;
        ...
        unsafe {
            esp!(wifi_prov_mgr_start_provisioning(
                ...
                service_key_cstr.as_ref()
                    .map(|cstr| cstr.as_ptr())
                    .unwrap_or(ptr::null()),
            ))?;
        }

        ::log::info!("🔵 BLE provisioning active - device discoverable as '{}'", device_name);
    ...
    fn print_qr_code(&self, device_name: &str, pop: Option<&str>) {
        ...
        let qr_payload = if let Some(pop_str) = pop {
            format!(r#"{{"ver":"v1","name":"{}","pop":"{}","transport":"ble"}}"#, device_name, pop_str)
        } else {
            format!(r#"{{"ver":"v1","name":"{}","transport":"ble"}}"#, device_name)
        };
        ...
        if let Ok(qr_code) = self.generate_ascii_qr(&qr_payload) {
            ...
            ::log::info!("🔗 Android: https://play.google.com/store/apps/details?id=com.espressif.provble");
        } else {
