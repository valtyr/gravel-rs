...
pub fn parse_scale_data(data: &[u8]) -> Option<ScaleData> {
    ...
    if data.len() != 20 {
        warn!(
            "Invalid data length: expected 20, got {} (Python expects exactly 20)",
            data.len()
        );
        warn!("Received data: {:02X?}", data);
        warn!(
            "This suggests we're reading from the wrong characteristic - need to find 0xFF11 UUID"
        );
        return None;
    }
    ...
    if data[0] != 0x03 || data[1] != 0x0B {
        warn!(
            "Invalid header: expected [0x03, 0x0B], got [{:02X}, {:02X}]",
            data[0], data[1]
        );
        warn!(
            "This suggests we're reading from the wrong characteristic - need to find 0xFF11 UUID"
