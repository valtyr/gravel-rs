use crate::system::storage::NvsStorage;
use crate::types::OVERSHOOT_HISTORY_SIZE;
...
impl OvershootController {
    ...
    pub async fn record_overshoot(
        ...
        debug!(
            "Overshoot check: pending={}, flow={:.2}g/s, weight={:.2}g, target={:.2}g",
        ...
        if flow_rate.abs() >= 0.5 {
            debug!(
                "Overshoot: Flow rate {:.2}g/s >= 0.5g/s - waiting for flow to stop",
                flow_rate
            );
            return;
        }
        ...
        if let Some(ref nvs) = self.nvs_storage {
            ...
            nvs.record_brew(overshoot, true, overshoot.abs() <= 1.0)
                .await;
        } else {
    ...
    async fn update_ewma_and_delay(&mut self, new_overshoot: f32) {
        ...
        self.ewma_overshoot =
            self.learning_rate * new_overshoot + (1.0 - self.learning_rate) * old_ewma;
    ...
    fn update_confidence(&mut self) {
        ...
        let variance: f32 = recent_overshoots
            .iter()
            .map(|&x| (x - mean).powi(2))
            .sum::<f32>()
            / recent_overshoots.len() as f32;
        ...
        debug!(
            "Confidence update: consistency={:.2}, experience={:.2}, combined={:.2}",
