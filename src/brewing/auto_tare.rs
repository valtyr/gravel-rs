use crate::types::{
    AutoTareState, BrewState, ScaleData, TARE_COOLDOWN_MS, TARE_STABILITY_THRESHOLD_G,
};
...
impl AutoTareController {
            ...
            AutoTareState::Loading => {
                if is_stable {
                    if is_empty {
                        ...
                        self.stable_weight = current_weight;
                        info!(
                            "AutoTare: Object stabilized: {:.1}g - TARING",
                            current_weight
                        );
                        return true;
                    }
    ...
    fn is_weight_stable(&mut self, current_weight: f32) -> bool {
        ...
        let recent_weights =
            &self.weight_history[self.weight_history.len() - self.stable_readings_needed..];
